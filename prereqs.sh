...
check_cmd mkfs.ext4    "rootfs image build + per-VM writable layers (e2fsprogs)"
check_cmd losetup      "attaching per-VM images to write into them (util-linux)"
check_cmd cpio         "overlay-root initrd build (update-firecracker.sh agent)"
check_cmd gzip         "overlay-root initrd compression"
...
check_opt dumpe2fs    "verifies the base image is clean before a read-only boot (e2fsprogs)"
check_opt fuser       "spots an image already attached to a running VM (psmisc); without it that check falls back to asking the Firecracker API"
...
if [ "$FAIL" -gt 0 ]; then
  ...
  echo "  - on Fedora:              sudo dnf install squashfs-tools e2fsprogs nftables nfs-utils cpio psmisc"
